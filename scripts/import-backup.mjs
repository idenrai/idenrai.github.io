...
td.addRule('colorscripter', {
  ...
  replacement: (_content, node) => {
    ...
    const codeDiv = node.querySelector?.('td:last-child div[style]') ||
                    (node.querySelectorAll ? Array.from(node.querySelectorAll('td'))[1] : null);
    if (!codeDiv) return '';
    const lines = [];
    Array.from(codeDiv.querySelectorAll ? codeDiv.querySelectorAll('div') : []).forEach((div) => {
...
td.addRule('tistory-figure', {
  ...
  replacement: (_content, node) => {
    const imgNodes = node.querySelectorAll ? Array.from(node.querySelectorAll('img')) : [];
    if (imgNodes.length === 0) return _content;
    return imgNodes.map((img) => {
      const src = img.getAttribute?.('src') || img.src || '';
      const alt = img.getAttribute?.('alt') || img.getAttribute?.('data-filename') || '';
      return src ? `\n\n![${alt}](${src})\n\n` : '';
    }).join('');
  },
});
...
td.addRule('local-img', {
  ...
  replacement: (_content, node) => {
    ...
    const alt = node.getAttribute?.('alt') || node.getAttribute?.('data-filename') || '';
    if (!src) return '';
    // 로컬 상대경로 이미지(백업 파일 내)는 빌드 오류 방지를 위해 제외
    if (src.startsWith('./') || src.startsWith('../') || (!src.startsWith('http') && !src.startsWith('/'))) return '';
    return `![${alt}](${src})`;
  },
});
...
async function main() {
  ...
  await import('jsdom').catch(() => {
    console.error('❌ jsdom이 없습니다. npm install jsdom --save-dev 실행 후 다시 시도하세요.');
    process.exit(1);
  });
  ...
  // 이미 생성된 tistory 포스트 번호 수집 (tistory-NNN.md 형태)
  const existing = new Set(
    (await fs.readdir(OUTPUT_DIR))
      .filter((f) => f.startsWith('tistory-') && f.endsWith('.md'))
      .map((f) => f.replace('tistory-', '').replace('.md', ''))
  );
  ...
  for (const filePath of files) {
    ...
    try {
      ...
      const frontmatter = makeFrontmatter({ title, date, tags, tistoryNum: folderNum });
      const fileContent = `${frontmatter}\n\n${markdown}\n`;

      const outPath = path.join(OUTPUT_DIR, `tistory-${folderNum}.md`);
  ...
  console.log(`\n🎉 완료!`);
  console.log(`   생성: ${created}개 / 건너뜀: ${skipped}개 / 오류: ${errors}개`);

  if (created > 0) {
    console.log('\n다음 단계:');
    console.log('  npm run build     — 빌드 확인');
    console.log('  git add . && git commit -m "feat: import all tistory posts" && git push');
  }
}
